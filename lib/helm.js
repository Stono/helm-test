...
module.exports = function Helm(exec) {
  ...
  self.version = function(version) {
    helmVersion = parseInt(version);
    if (helmVersion !== 2 && helmVersion !== 3) {
      throw new Error(`Unexpected helm version ${version}: valid values ["2", "3"]`);
    }
    return self;
  };
  ...
  self.go = function(done) {
    let command = commands[helmVersion];
    if(files.length > 0) {
