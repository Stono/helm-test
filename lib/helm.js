...
module.exports = function Helm(exec) {
  const helmDefaultCommand = 'helm template --namespace default --name release-name .';
  let self = {};
  let files = [];
  let sets = [];
  let command = helmDefaultCommand;

  self.version = function(version) {
    switch(version.toString()) {
      case '3':
        command ='helm template --namespace default release-name .';
        break;
      case '2':
        command = helmDefaultCommand;
        break;
      default:
        throw `Unexpected helm version ${version}: valid values ["2", "3"]`;
    }
    return self;
  };
